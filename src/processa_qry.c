...
static void cm(double x, double y, double w, double h, double dx, double dy, Arvore formas, Arvore formas_marcadores)
{
...
static void comando_cm(const char *linha, FILE *fp_qry, FILE *fp_log, Arvore formas, Arvore formas_marcadores)
{
...
static void sel(double x, double y, double w, double h, Arvore formas, Arvore formas_marcadores)
{
	Forma retangulo_sel = cria_forma('r', cria_retangulo(ID_RETANGULO_SEL, x, y, w, h, CORB_RETANGULO_SEL, CORP_RETANGULO_SEL));
	insere_arvore(formas_marcadores, retangulo_sel);
...
static void comando_sel(const char *linha, FILE *fp_qry, FILE *fp_log, Arvore formas, Arvore formas_marcadores)
{
	double x, y, w, h;
...
static void cria_quadrados_marcadores(Arvore formas_marcadores, int k)
{
	for (int i = 0; i < k; i++)
	{
		...
		Forma quadrado_ancora = cria_forma('r', cria_retangulo(ID_QUADRADO_MARCA, x, y, LADO_QUADRADO_MARCA, LADO_QUADRADO_MARCA, CORB_QUADRADO_MARCA, CORP_QUADRADO_MARCA));
...
static void find(FILE *fp_log, int k, char *alg, char crit, double x, double y, double dw, const char *comb_out, Arvore formas, Arvore formas_marcadores, bool rm)
{
	...
	if (strcmp(alg, "bs") == 0)
		bubble_sort_animado(comb_out, vet_sel, n_selecionadas, k, criterio_ordenacao);
	/*
	...
	*/
	...
	if (rm)
		remove_formas_maiores(vet_sel, k);
}

static void comando_find(const char *linha, const char *comb_out, FILE *fp_qry, FILE *fp_log, Arvore formas, Arvore formas_marcadores, bool rm)
{
	int k;
	char alg[8];
	char crit;
