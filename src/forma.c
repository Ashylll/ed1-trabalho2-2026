...
void reporta_forma(FILE* fp_txt, Forma f, const char criterio_ordenacao){
    ...
    switch (criterio_ordenacao){
        case 'd':
            fprintf(fp_txt, "== %s ==\nId: %d\nÂncora: y = %.2lf || x = %.2lf\nÁrea: %.2lf\n",
            tipo, get_id_forma(f), get_y_forma(f), get_x_forma(f), get_area_forma(f));

            break;
        case 'a':
            fprintf(fp_txt, "== %s ==\nId: %d\nÁrea: %.2lf\n",
            tipo, get_id_forma(f), get_area_forma(f));
            
            break;
        case 'w': 
            fprintf(fp_txt, "== %s ==\nId: %d\nLargura: %.2lf\n",
            tipo, get_id_forma(f), get_largura_forma(f));

            break;
        case 'h':
            fprintf(fp_txt, "== %s ==\nId: %d\nAltura: %.2lf\n",
            tipo, get_id_forma(f), get_altura_forma(f)); 
        ...
        case 'c': 
            if (t == 'l'){
                ...
                tipo, get_id_forma(f), get_corp_forma(f));
            }
    ...
    *dy = 0;
    ...
    switch (get_tipo_forma(f)) {
        ...
        case 't': { 
            ...
            if (ancora == 'm') *dx = get_largura_x_forma(f) / 2.0;
            else if (ancora == 'f') *dx = get_largura_x_forma(f);
            break;
        }
        case 'l': {
...
double get_area_forma(Forma f){
    ...
    switch (tipo){
        case 'c': return get_area_circulo(handle);
        case 'r': return get_area_retangulo(handle);
        case 't': return get_area_texto(handle);
        case 'l': return get_area_linha(handle);
        default: return -2;
    }
}
...
double get_largura_forma(Forma f){
    ...
    switch (tipo){
        case 'c': return 2 * get_raio_circulo(handle);
        case 'r': return get_w_retangulo(handle);
        case 't': return strlen(get_palavra_texto(handle));
        case 'l': return get_comprimento_linha(handle);
        default : return -2;
    }
}
...
double get_altura_forma(Forma f){
    ...
    switch (tipo){
        case 'c': return 2 * get_raio_circulo(handle);
        case 'r': return get_h_retangulo(handle);
        case 't': return 10.0;
        case 'l': return 1.5;
        default : return -2;
    }
...
char* get_corb_forma(Forma f){
    ...
    switch (tipo){
        case 'c': return get_corb_circulo(handle);
        case 'r': return get_corb_retangulo(handle);
        case 't': return get_corb_texto(handle);
        case 'l': return get_cor_linha(handle);
        default : return NULL;
    }
}
...
char* get_corp_forma(Forma f){
    ...
    switch (tipo){
        case 'c': return get_corp_circulo(handle);
        case 'r': return get_corp_retangulo(handle);
        case 't': return get_corp_texto(handle);
        case 'l': return get_cor_linha(handle);
        default : return NULL;
    }
}
...
void set_x_forma(Forma f, double x){
    ...
    void* handle = get_handle_forma(f);
    switch (forma->tipo){
        case 'c': set_x_circulo(handle, x);
        case 'r': set_x_retangulo(handle, x);
        case 't': set_x_texto(handle, x);
        case 'l': set_ancora_linha(handle, x, get_y1_linha(handle));
...
void set_y_forma(Forma f, double y){
    ...
    void* handle = get_handle_forma(f);
    switch (forma->tipo){
        case 'c': set_y_circulo(handle, y);
        case 'r': set_y_retangulo(handle, y);
        case 't': set_y_texto(handle, y);
        case 'l': set_ancora_linha(handle, get_x1_linha(handle), y);
