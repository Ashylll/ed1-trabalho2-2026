...
static bool comando_c(const char *linha, Arvore a) {
  ...
  if (sscanf(linha, "%*s %d %lf %lf %lf %31s %31s", &i, &x, &y, &r, corb,
             corp) != 6)
    return false;
...
static bool comando_r(const char *linha, Arvore a) {
  ...
  if (sscanf(linha, "%*s %d %lf %lf %lf %lf %31s %31s", &i, &x, &y, &w, &h,
             corb, corp) != 7)
    return false;
...
static bool comando_l(const char *linha, Arvore a) {
  ...
  if (sscanf(linha, "%*s %d %lf %lf %lf %lf %31s", &i, &x1, &y1, &x2, &y2,
             cor) != 6)
    return false;

  Linha l = NULL;
  if (x1 != x2)
    l = (x1 < x2) ? cria_linha(i, x1, y1, x2, y2, cor)
                  : cria_linha(i, x2, y2, x1, y1, cor);
  else
    l = (y1 < y2) ? cria_linha(i, x1, y1, x2, y2, cor)
                  : cria_linha(i, x2, y2, x1, y1, cor);
...
static bool comando_ts(const char *linha) {
  ...
  if (sscanf(linha, "%*s %31s %7s %lf", novo_family, novo_weight, &novo_size) !=
      3)
    return false;
